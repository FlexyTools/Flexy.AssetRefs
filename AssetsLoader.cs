...
namespace Flexy.AssetRefs
{
	public abstract class AssetsLoader
	{
		...
		public				LoadTask<Scene>			LoadSceneAsync				( SceneRef @ref, GameObject context )	
		{
			var op = GenericPool<LoadTaskData>.Get( );
			
#if UNITY_EDITOR			
			if( AssetRef.AllowDirectAccessInEditor )
				return new ( LoadSceneInEditorAsync( this, context, @ref, op ), op );
#endif
			return new( LoadSceneInternalAsync( this, context, @ref, op ), op );
			
#if UNITY_EDITOR
			static async UniTask<Scene> LoadSceneInEditorAsync( AssetsLoader loader, GameObject context, SceneRef @ref, LoadTaskData op )
			{
				try
				{
					...
					var sceneLoadOp	= UnityEditor.SceneManagement.EditorSceneManager.LoadSceneAsyncInPlayMode( path, new LoadSceneParameters { loadSceneMode = LoadSceneMode.Additive } );
			...
			static async UniTask<Scene> LoadSceneInternalAsync( AssetsLoader loader, GameObject context, SceneRef @ref, LoadTaskData op )
			{
				try
				{
					...
					var sceneLoadOp		= loader.LoadSceneAsync_Impl( @ref );
		...
		public 				LoadTask<T>				LoadSceneAsync<T>			( SceneRef @ref, GameObject context ) where T: MonoBehaviour	
		{
			var sceneLoadTask	= LoadSceneAsync( @ref, context );
		...
		protected abstract 	AsyncOperation			LoadSceneAsync_Impl			( SceneRef @ref );
	}
